...
shinyServer(function(input, output,session) {
  ...
  output$powerPicPlot<-renderPlot({
    ...
    #determine which regions to shade and shade them
  ...
  #create info of Power and prob of type II error
  output$typeII<-renderUI({
    #get inputs
    mu0<-input$mu0
    n<-input$sampleSize
    sigma<-input$sigma
    ...
    list(withMathJax(HTML(paste0('$$\\alpha = ',alpha,'$$'))),
         withMathJax(HTML(paste0('$$\\beta = ',round(beta,4),'$$'))),
         withMathJax(HTML(paste0('$$Power = 1-\\beta = ',1-round(beta,4),'$$')))
    )
    
  })
  
  
  #get distributions under H0/HA
  output$dists<-renderUI({
    #get inputs
    mu0<-input$mu0
    n<-input$sampleSize
    sigma<-input$sigma
    ...
    list(withMathJax(HTML(paste0('$$\\bar{Y}\\stackrel{H_0}\\sim N(\\mu_0,\\sigma^2/n)=N(',mu0,',',sigma^2,'/',n,')=N(',mu0,',',round(sigma^2/n,4),')$$'))),
      withMathJax(HTML(paste0('$$\\bar{Y}\\stackrel{H_A}\\sim N(\\mu_A,\\sigma^2/n)=N(',muA,',',sigma^2,'/',n,')=N(',muA,',',round(sigma^2/n,4),')$$'))))
